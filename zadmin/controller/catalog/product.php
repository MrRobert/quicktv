...
class ControllerCatalogProduct extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if(isset($this->request->get['indexProductId']) && isset($this->session->data['lstProductId'])){
                $index = $this->request->get['indexProductId'];
                $this->response->redirect($this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] .'&indexProductId='. ($index + 1) . '&product_id=' . $this->session->data['lstProductId'][$index] . $url, 'SSL'));
            }else{
                $this->response->redirect($this->url->link('catalog/product', 'token=' . $this->session->data['token'] . $url, 'SSL'));
            }
        }
	...
	protected function getList() {
		...
		foreach ($results as $result) {
            if($result['status'] == 0){
                $this->session->data['lstProductId'][] = $result['product_id'];
                $editLink = $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] .'&indexProductId='. ($index + 1) . '&product_id=' . $result['product_id'] . $url, 'SSL');
                $index ++;
            }else{
                $editLink = $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL');
            }
